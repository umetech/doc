...
-  返回的主节点中的 timestamp=1608261650, 应用应当保存好，下次发起数据同步时，需要使用

### 2. 数据同步

> 当应用有新增、更新或删除操作时，发起数据同步
...
- 数据同步，主节点timestamp为应用保存的最新同步时间戳（记住，这个值不能为0，为0表示初始化同步）
- POST的数据中，flag=1表示新增的数据, flag=2表示更新的数据，flag=4表示删除的数据
...
其它条件服务端将**不会进行处理**，直接返回为空对象(即[])

### 2. 重复数据提交的处理

​	当Client重复提交相同数据时，服务端会将返回的结果与第一次提交时返回的结果一致
